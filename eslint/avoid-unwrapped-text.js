...
function getTagName(node) {
	...
	if (node.type === 'JSXElement' && node.openingElement.type === 'JSXOpeningElement') {
		...
		return {
			...
			Literal(node) {
				...
				while (parent) {
					...
					if (parent.type === 'JSXExpressionContainer' || parent.type === 'LogicalExpression') {
						parent = parent.parent;
						continue;
					}
			...
			ReturnStatement(node) {
				...
				if (!isTextComponent(returnedComponentName)) {
					context.report({
						node,
						message: 'Components ending with *Text must return <Text> or <SomeText>.',
					});
				}
			},
		};
