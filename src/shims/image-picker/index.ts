...
export async function launchImageLibraryAsync(options?: ImagePickerOptions): Promise<ImagePickerResult> {
	return new Promise((resolve) => {
		...
		input.multiple = Boolean(
			options?.allowsMultipleSelection || (options?.selectionLimit && options.selectionLimit > 1),
		);
		...
		input.addEventListener('change', async () => {
			...
			const selected = limit && limit > 0 && input.multiple ? files.slice(0, limit) : files;

			try {
				const assets = await Promise.all(selected.map((file) => fileToAsset(file, options)));
...
function getAccept(options?: ImagePickerOptions) {
	const mediaTypes = Array.isArray(options?.mediaTypes) ? options.mediaTypes : undefined;
...
async function fileToAsset(file: File, options?: ImagePickerOptions): Promise<ImagePickerAsset> {
...
function getImageDimensions(uri: string): Promise<{ width: number; height: number; duration: null }> {
	return new Promise((resolve, reject) => {
...
function getVideoDimensions(file: File): Promise<{ width: number; height: number; duration: number | null }> {
	return new Promise((resolve, reject) => {
