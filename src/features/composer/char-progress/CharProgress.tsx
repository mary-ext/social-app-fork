...
export function CharProgress({ count, className }: { count: number; className?: string }) {
	const isOverLimit = count > MAX_POST_GRAPHEME_LENGTH;
	...
	return (
		<div className={clsx(styles.container, className)}>
			<Text size="md_sub" className={clsx(styles.count, isOverLimit && styles.countOver)}>
				{MAX_POST_GRAPHEME_LENGTH - count}
			</Text>
			...
			{isOverLimit ? (
				<ProgressPie
					...
					borderWidth={4}
					color={circleColor}
					progress={Math.min((count - MAX_POST_GRAPHEME_LENGTH) / MAX_POST_GRAPHEME_LENGTH, 1)}
					size={20}
				/>
			) : (
				<ProgressCircle
