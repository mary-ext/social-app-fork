...
/**
 * The pair of XRPC clients backing every network call. `appview` reaches the Bluesky AppView; `pds` reaches
 * the signed-in user's PDS and is `null` while logged out (no session, no PDS).
 */
...
/**
 * Builds the logged-out client pair. `public.api.bsky.app` is itself the AppView, so `appview` needs no
 * proxy; there is no session, so `pds` is `null`.
 *
...
/**
 * Builds the logged-in client pair. Both clients share the OAuth handler targeting the user's PDS; `appview`
 * additionally carries the `#bsky_appview` proxy header so the PDS forwards to the AppView.
 *
