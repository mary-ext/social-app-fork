...
export class Convo {
	...
	subscribe(subscriber: () => void) {
		if (this.subscribers.length === 0) {
			this.cleanupProfileShadowListener = listenProfileShadowUpdate(({ did, shadow }) => {
	...
	private applyProfileShadow(did: string, shadow: Partial<ProfileShadow>): boolean {
		let changed = false;
...
function applyShadowToConvo(
	...
	if (convo.kind === 'group') {
		...
		return {
			...convo,
			members,
			primaryMember: convo.primaryMember?.did === did ? merged : convo.primaryMember,
		};
	} else {
		const target = convo.members[i]!;
		const merged = mergeShadow(target, shadow);
