...
type StateContext = {
	debugFeedContextEnabled: Exclude<persisted.Schema['debugFeedContextEnabled'], undefined>;
};
type ApiContext = {
	setDebugFeedContextEnabled(enabled: Exclude<persisted.Schema['debugFeedContextEnabled'], undefined>): void;
};
...
function usePersistedBooleanValue<T extends keyof persisted.Schema>(key: T) {
	...
	const set = useCallback<(value: Exclude<persisted.Schema[T], undefined>) => void>(
		(enabled) => {
...
export function Provider({ children }: React.PropsWithChildren<{}>) {
	...
	const state = useMemo(() => ({ debugFeedContextEnabled }), [debugFeedContextEnabled]);
	const api = useMemo(() => ({ setDebugFeedContextEnabled }), [setDebugFeedContextEnabled]);
