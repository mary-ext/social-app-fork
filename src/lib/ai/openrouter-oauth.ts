...
import { OPENROUTER_PROVIDER_NAME } from '#/lib/ai/config';
...
const fail = createProviderError(OPENROUTER_PROVIDER_NAME);
 ...
 * @returns an API key
 * @throws {AiProviderError} if no verifier exists or the exchange fails
 */
export const exchangeOpenRouterCode = async (code: string): Promise<string> => {
	const verifier = sessionStorage.getItem(VERIFIER_KEY);
	if (verifier === null) {
		throw fail('no OpenRouter sign-in was started in this tab');
	}
	...
	try {
		...
		throw fail('could not reach OpenRouter', { cause: error });
	}

	const body: unknown = await response.json().catch(() => undefined);

	if (!response.ok) {
		const failure = v.safeParse(aiErrorEnvelopeSchema, body);
		const detail = failure.success ? failure.output.error.message : undefined;
		throw fail(detail ?? `OpenRouter returned ${response.status}`, { status: response.status });
	}

	const parsed = v.safeParse(keySchema, body);
	if (!parsed.success) {
		throw fail('OpenRouter returned a reply in an unexpected shape');
	}
