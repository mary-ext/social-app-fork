...
export type TranslationFunctionParams = {
	...
	/**
	 * We auto-detect the source language by default, but the user has the option to specify a source language
	 * if they want to. If this value is present, it means the user selected a source language, or we were
	 * certain of the source language and want to specify it explicitly.
	 */
	expectedSourceLanguage?: string;
	/**
	 * The languages the content might be in, such as the user-supplied language codes on posts. Currently only
	 * available on posts.
	 */
...
export type TranslationFunction = (params: TranslationFunctionParams) => Promise<void>;

export type ContextType = {
	translationState: Record<string, TranslationState>;
	translate: (params: TranslationFunctionParams, options: TranslationOptions) => Promise<void>;
