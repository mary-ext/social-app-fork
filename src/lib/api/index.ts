...
export async function post(
	...
	for (let i = 0; i < thread.posts.length; i++) {
		...
		const embedPromise = resolveEmbed(agent, queryClient, draft, opts.onStateChange, featureFlags);
...
> {
	if (draft.embed.quote) {
		const [resolvedMedia, resolvedQuote] = await Promise.all([
			resolveMedia(agent, queryClient, draft.embed, onStateChange, featureFlags),
	...
	const resolvedMedia = await resolveMedia(agent, queryClient, draft.embed, onStateChange, featureFlags);
	if (resolvedMedia) {
