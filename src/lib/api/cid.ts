 ...
 *
 * The record must carry its `$type`: the CID is computed over the DAG-CBOR encoding with the field
 * present, even though the PDS accepts record writes without it. `@atcute/cbor` natively encodes
 * blob refs, CID links, and bytes, and drops `undefined` values.
 *
