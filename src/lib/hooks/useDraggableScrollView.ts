...
export function useDraggableScroll<Scrollable extends ScrollView = ScrollView>({
	...
	useEffect(() => {
		...
		const mouseMove = (e: MouseEvent) => {
			...
			if (slider.contains(document.activeElement)) (document.activeElement as HTMLElement)?.blur?.();
		};
	...
	// oxlint-disable-next-line react/react-compiler - RC doesn't know this is heading to the `ref `prop
