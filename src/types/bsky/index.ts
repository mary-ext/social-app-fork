 ...
 * cases. Why? Our SDK's `is*` identity utils do not assert the type of the entire object, only the `$type`
 * string.
 *
 * For full validation of the object schema, use the `validate` export from this file.
 *
 * Usage:
 *
 * ```ts
 ...
 *
 * For faster checks with data we trust, like that from our app view, use the `dangerousIsType` export from
 * this same file.
 *
 * Usage:
 *
