...
import { type ScrollEvent, useScrollHandlers } from '#/lib/ScrollContext';
...
export type ListMethods = {
	...
	// Signature kept compatible with FlatList's scrollToIndex so callers stay
	// platform-agnostic. viewOffset/viewPosition are accepted for parity but not
...
function ListImpl<ItemT>(
	...
	const disableFullWindowScroll = disableFullWindowScrollProp ?? isWithinSplitView;
	...
	const registerRowNode = useCallback((index: number, node: HTMLElement | null) => {
		if (node) {
	...
	useImperativeHandle(
		...
		() => ({
			...
			scrollToIndex({ animated = true, index }) {
				...
				// `block: 'center'` roughly matches the caller's viewPosition of 0.3 -
				// not exact, but close enough, and it respects whichever element is the
				// scroll container (window or nativeRef).
				node?.scrollIntoView({
					block: 'center',
					behavior: animated ? 'smooth' : 'instant',
				});
...
let Row = function RowImpl<ItemT>({
	...
	item: ItemT;
	index: number;
	renderItem: null | undefined | ((info: ListRenderItemInfo<ItemT>) => React.ReactNode);
	extraData: unknown;
