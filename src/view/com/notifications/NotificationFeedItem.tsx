...
let NotificationFeedItem = ({
	...
	if (item.type === 'post-like') {
		...
		icon = <ContactsIconFilled size="xl" style={{ color: t.palette.primary_500 }} />;
...
function FollowBackButton({ profile }: { profile: AppBskyActorDefs.ProfileView }) {
	...
	const onPressFollow = async (e: GestureResponderEvent) => {
		...
		try {
			await queueFollow();
			Toast.show(l`Following ${sanitizeDisplayName(profile.displayName || profile.handle)}`);
		} catch (err) {
	...
	const onPressUnfollow = async (e: GestureResponderEvent) => {
		...
		try {
			await queueUnfollow();
			Toast.show(l`No longer following ${sanitizeDisplayName(profile.displayName || profile.handle)}`);
		} catch (err) {
			if (!(err instanceof Error && err.name === 'AbortError')) {
				Toast.show(l`An issue occurred, please try again.`, {
					type: 'error',
	...
	if (!visible) {
		return (
			<View style={[a.flex_row, a.align_center]}>
				...
				>
					<ChevronUpIcon size="md" style={[a.ml_xs, a.mr_md, t.atoms.text_contrast_high]} />
	...
	return (
		<TouchableOpacity accessibilityRole="none" onPress={onToggleAuthorsExpanded}>
			<View style={[a.flex_row, a.align_center]}>
				...
				<ChevronDownIcon size="md" style={[a.mx_xs, t.atoms.text_contrast_medium]} />
			</View>
		</TouchableOpacity>
	);
}
