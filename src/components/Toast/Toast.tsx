...
export function ToastConfigProvider({
	...
	return (
		<ToastConfigContext.Provider value={useMemo(() => ({ id, type }), [id, type])}>
			{children}
						...
						<>
							...
							<BaseText
								style={[
									...
									a.leading_snug,
									{
										color: interacted ? styles.interacted.textColor : styles.base.textColor,
									},
								]}
							>
								{props.children}
...
/**
 * Vibes-based number, provides t `top` value to wrap the text to compensate for different type sizes and keep
 * the first line of text aligned with the icon. - esb
 */
