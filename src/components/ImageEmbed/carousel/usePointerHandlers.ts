...
export function usePointerHandlers({
	...
	useEffect(() => {
		...
		const onMouseUp = () => {
			...
			if (wasDragging) {
				// Suppress the click that follows mouseup after a drag so it doesn't open the lightbox. Scoped to
				// `el`: a drag releasing on the post body navigates the row otherwise, but `BlockLink` already
				// ignores clicks whose press began inside the carousel, so we only need to swallow clicks landing
		...
		/*
		 * Safari does not support `overscroll-behavior`, so a horizontal trackpad swipe over the carousel
		 * can trigger the browser's back/forward navigation gesture. We intercept predominantly-horizontal
		 * wheel events and apply the scroll ourselves, calling preventDefault to suppress the history-nav
		 * gesture. Vertical-dominant wheel events are left untouched so normal page scroll still works.
		 * Chrome/Firefox are covered by the `overscrollBehaviorX: 'contain'` style on the scroll viewport.
		 *
		...
		const onWheel = (e: WheelEvent) => {
			...
			// Keep the active index in sync so keyboard/lightbox stay correct, but only settle when it
			// actually changes - onSettle moves focus, which we don't want to thrash on every wheel tick.
			let accumulated = 0;
			let index = 0;
			for (let i = 0; i < imageCount; i++) {
				const w = (itemWidthsRef.current.get(i) ?? 0) + ITEM_GAP;
