...
export function AutosizedTextarea({
	...
	const normalizedStyles = normalizeTextStyles([a.text_md, a.leading_snug, t.atoms.text, outerStyle], {
	...
	const maxInputHeight = maxRows ? lineHeight * maxRows + verticalContentPadding : Infinity;
	...
	const handleResizeWeb = () => {
		...
		const nextHeight = Math.min(Math.max(scrollHeight, minInputHeight), maxInputHeight);
	...
	const onContentSizeChange = (e: TextInputContentSizeChangeEvent) => {
		...
		const nextHeight = Math.min(Math.max(height, minInputHeight), maxInputHeight);
