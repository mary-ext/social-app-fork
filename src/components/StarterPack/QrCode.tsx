...
export function QrCode({
	...
	return (
		...
		>
			<View style={[a.gap_sm]}>
				<Text style={[a.font_semi_bold, a.text_3xl, a.text_center, { color: 'white' }]}>{record.name}</Text>
			</View>
			<View style={[a.gap_xl, a.align_center]}>
				<Text style={[a.font_semi_bold, a.text_center, { color: 'white', fontSize: 18 }]}>
...
export function QrCodeInner({ link }: { link: string }) {
	...
	const onLogoAreaChange = (area: { x: number; y: number; width: number; height: number }) => {
		setLogoArea(area);
	};
