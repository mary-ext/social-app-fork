...
export type StackedButtonProps = Omit<ButtonProps, keyof VariantProps | 'children'> &
...
export function StackedButton({ children, ...props }: StackedButtonProps) {
	return (
		...
		>
			<StackedButtonInnerText icon={props.icon}>{children}</StackedButtonInnerText>
		</Button>
	);
}

function StackedButtonInnerText({ children, icon: Icon }: Pick<StackedButtonProps, 'icon' | 'children'>) {
	const textStyles = useSharedButtonTextStyles();
	return (
		<>
