import { createContext, type CSSProperties, forwardRef, Fragment, useContext, useMemo } from 'react';
			...
			<RadixSelect.Trigger
				...
				style={flatten([
					...
					a.pointer,
					{
						...
						borderWidth: 1,
						borderStyle: 'solid',
						borderColor: focused ? t.palette.primary_500 : t.palette.contrast_50,
					},
				])}
			>
				{children}
...
export function ValueText({ children, webOverrideValue, style, ...props }: ValueProps) {
	let content;
...
export const ItemText = function ItemText({ children, style }: ItemTextProps) {
	return (
		<RadixSelect.ItemText asChild>
			<Text style={flatten([style, { pointerEvents: 'inherit' } as WebTextStyle])}>{children}</Text>
...
export function ItemIndicator({ icon: Icon = CheckIcon }: ItemIndicatorProps) {
	return (
		<RadixSelect.ItemIndicator
			style={flatten([a.absolute, { left: 0, width: 30 }, a.flex, a.align_center, a.justify_center])}
		>
			<Icon size="sm" />
		</RadixSelect.ItemIndicator>
	);
