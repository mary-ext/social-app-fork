...
function VideoCards({ data }: { data: Exclude<ReturnType<typeof usePostFeedQuery>['data'], undefined> }) {
	const items = useMemo(() => {
		return data.pages
		...
		<>
			{items.map((item) => (
				<View key={item.post.uri} style={[{ width: CARD_WIDTH }]}>
					<CompactVideoPostCard post={item.post} moderation={item.moderation} onInteract={() => {}} />
				</View>
			))}
			<ViewMoreCard />
		</>
			...
			>
				{({ pressed }) => (
					...
					>
						...
						<Button color="primary" size="small" shape="round" label={l`View more trending videos`}>
