...
export function TranslatedPost({
	...
	switch (translationState.status) {
		...
		default:
			return (
				needsTranslation && (
					<TranslationLink translate={translate} initialTranslationParams={initialTranslationParams} />
				)
			);
	}
}
...
function TranslationLink({
	...
	return (
		<View style={[a.gap_md, a.mt_sm, a.align_start, a.flex_row, a.align_center, a.gap_xs]}>
			<Link
				role={'link'}
				{...createStaticClick(() => {
					handleTranslate();
...
function TranslationError({
	...
	const hoverUnderlineStyle = [a.underline, { textDecorationColor: t.palette.primary_500 }];

	return (
		<View style={[a.p_md, a.mt_sm, a.border, a.rounded_lg, a.gap_xs, t.atoms.border_contrast_high]}>
			<View
				style={[
					a.flex_row,
					a.align_start,
			...
			>
				...
				<Text style={[a.flex_1, a.text_xs, a.leading_snug, t.atoms.text_contrast_high]}>{message}</Text>
			...
			<View style={[a.flex_row, a.align_center]}>
				<Link
					...
					hitSlop={HITSLOP_30}
				>
					<Text style={[a.text_xs, a.font_medium, a.leading_snug, { color: t.palette.primary_500 }]}>
...
function TranslationResult({
	...
	const langName = resultSourceLanguage ? codeToLanguageName(resultSourceLanguage, i18n.locale) : undefined;
		...
		<View>
			<View style={[a.p_md, a.mt_sm, a.border, a.rounded_lg, a.gap_xs, t.atoms.border_contrast_high]}>
				...
				>
					{langName ? (
						<>
							<Text style={[a.text_xs, a.leading_snug, t.atoms.text_contrast_medium]}>{langName} </Text>
							<ArrowRightIcon size="xs" fill={t.atoms.text_contrast_medium.color} />
							<Text style={[a.text_xs, a.leading_snug, t.atoms.text_contrast_medium]}>
								{' '}
								{codeToLanguageName(langPrefs.primaryLanguage, langPrefs.appLanguage)}
							</Text>
						</>
					) : (
						<Text style={[a.text_xs, a.leading_snug, t.atoms.text_contrast_medium]}>
					...
					{resultSourceLanguage != null && (
						<>
							<Text style={[a.text_xs, a.font_medium, a.leading_snug, t.atoms.text_contrast_medium]}>
								{' '}
								&middot;{' '}
							</Text>
...
function TranslationLanguageSelect({
	...
	return (
		<Select.Root value={resultSourceLanguage} onValueChange={handleChangeTranslationLanguage}>
			<Select.Trigger label={l`Change the source language`}>
				{({ props }) => {
					return (
						<Button label={props.accessibilityLabel} {...props} hitSlop={HITSLOP_30}>
							<Text style={[a.text_xs, a.font_medium, a.leading_snug, t.atoms.text_contrast_high]}>
								<Trans>Change</Trans>
							</Text>
						</Button>
					);
