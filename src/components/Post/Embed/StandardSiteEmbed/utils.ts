import { type AppBskyEmbedExternal, AtUri, type ComAtprotoRepoStrongRef } from '@atproto/api';
...
export function isStandardSitePublicationEmbed(view: AppBskyEmbedExternal.ViewExternal) {
	return (
		view.associatedRefs?.some((ref) => new AtUri(ref.uri).collection === 'site.standard.publication') &&
		view.associatedRefs.every((ref) => new AtUri(ref.uri).collection !== 'site.standard.document')
	);
}
