...
export const IconTemplate_Stroke2_Corner0_Rounded = forwardRef<SVGSVGElement, Props>(
	function IconImpl(props, ref) {
		const { fill, gradient, size, style, ...rest } = useCommonSVGProps(props);
...
export function createSinglePathSVG({
	...
	return forwardRef<SVGSVGElement, Props>(function IconImpl(props, ref) {
		const { fill, gradient, size, style, ...rest } = useCommonSVGProps(props);
