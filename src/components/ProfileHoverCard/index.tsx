...
export function ProfileHoverCard(props: ProfileHoverCardProps) {
	...
	if (props.disable || IS_WEB_TOUCH_DEVICE) {
		...
		return (
			<View onPointerMove={onPointerMove} style={[a.flex_shrink, props.inline && a.inline, props.style]}>
...
export function ProfileHoverCardInner(props: ProfileHoverCardProps) {
	...
	const [currentState, dispatch] = useReducer(
		...
		(state: State, action: Action): State => {
			...
			function showing({ reason }: { reason: 'hovered-target' | 'hovered-card' }): State {
				return {
					stage: 'showing',
					reason,
					effect() {
	...
	return (
		...
		>
			{data && moderationOpts ? (
				...
				<View style={[a.justify_center, a.align_center, { minHeight: 200 }, a.w_full]}>
					<Loader size="xl" />
				</View>
			)}
		</View>
