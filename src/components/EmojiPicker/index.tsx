 ...
 *
 * On emoji select, fires a `textInputWebEmitter` event (for web text inputs that listen for emoji insertions)
 * and forwards to the optional `onEmojiSelect` callback.
 *
 * @platform web
 */
export function Root({ children, control, onEmojiSelect, preloadOnMount = true, nextFocusRef }: RootProps) {
...
/**
 * Passthrough to {@link Menu.Trigger}. Accepts the same render-prop children pattern.
 *
 * @platform web
 */
 ...
 *
 * Holding Shift while selecting an emoji keeps the picker open for multi-select. Otherwise the menu closes
		...
		<DropdownMenu.Portal>
			<DropdownMenu.Content
				...
				onCloseAutoFocus={(evt) => {
					...
					let element = nextFocusRef instanceof Function ? nextFocusRef() : nextFocusRef.current;
					if (element) {
						evt.preventDefault();
						element.focus();
					}
