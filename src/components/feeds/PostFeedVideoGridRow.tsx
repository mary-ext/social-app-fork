...
export function PostFeedVideoGridRow({ items: slices }: { items: FeedPostSliceItem[] }) {
	const gutters = useGutters(['base', 'base', 0, 'base']);
	const posts = slices
		.filter((slice) => slice.post.embed?.$type === 'app.bsky.embed.video#view')
		.map((slice) => ({
			post: slice.post,
	...
	/**
	 * This should not happen because we should be filtering out posts without videos within the `PostFeed`
	 * component.
	 */
				...
				<Grid.Row gap={a.gap_sm.gap}>
					{posts.map((post) => (
						<Grid.Col key={post.post.uri} width={1 / 2}>
							<VideoPostCard post={post.post} moderation={post.moderation} onInteract={() => {}} />
						</Grid.Col>
					))}
				</Grid.Row>
			</View>
