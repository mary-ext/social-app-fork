...
type WebInputStyle = Omit<
	TextStyle,
	'background' | 'caretColor' | 'overscrollBehavior' | 'scrollbarColor' | 'scrollbarWidth'
> & {
...
/**
 * Imperative API exposed via `internalApiRef` prop for parent components that need to control the composer
 * programmatically, e.g. to clear the input or insert text at the current cursor position.
 */
...
export type ComposerProps = Omit<
	...
	'value' | 'onChange' | 'onChangeText' | 'onSelectionChange' | 'selection' | 'style' | 'onSubmitEditing'
> & {
	label: string;
	...
	autocompletePlacement?: Exclude<Parameters<typeof useSift>[0], undefined>['placement'];
