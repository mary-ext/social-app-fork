...
function LightboxGallery({
	...
	useEffect(() => {
		...
		return () => {
			...
			if (!closedByPopStateRef.current && (history.state as { lightbox?: boolean })?.lightbox) {
				history.back();
			}
		};
	}, [onClose]);
		...
		<View style={[a.absolute, a.inset_0]}>
			<View style={[a.flex_1, a.justify_center, a.align_center]}>
				...
				{canGoLeft && (
					...
					>
						<ChevronLeftIcon size={gtPhone ? 'md' : 'sm'} style={{ color: t.palette.white }} />
					</Button>
				)}
				{canGoRight && (
					<Button
						...
						size={gtPhone ? 'large' : 'small'}
					>
						<ChevronRightIcon size={gtPhone ? 'md' : 'sm'} style={{ color: t.palette.white }} />
					</Button>
				)}
			</View>
			{img.alt ? (
			...
			<View style={[a.absolute, styles.closeBtn, delayedFadeInAnim]}>
				<CircleChromeButton icon={XIcon} label={l`Close image viewer`} onPress={onClose} />
			</View>
			{imgs.length > 1 && (
				<View style={[a.absolute, styles.pagerDots, delayedFadeInAnim]} pointerEvents="none">
...
function LightboxGalleryItem({
	...
	/**
	 * We want to show a zoom/fade in animation when the lightbox first opens. To avoid showing it as we switch
	 * between images, we keep track in the parent whether any image has loaded yet. We then save what the value
	 * of this is on first render (as when it changes, we don't want to then _remove_ then animation). when the
	 * image loads, if this is the first image to load, we play the animation.
	 *
	 * We also use this `hasLoaded` state to show a loading indicator. This is on a 1s delay and then a slow
	 * fade in to avoid flicker. -sfn
	 */
	const zoomInWhenReady =
		!reduceMotionEnabled &&
		isFirstToLoad &&
		(hasAnyLoaded ? [a.zoom_fade_in, { animationDuration: '0.5s' }] : { opacity: 0 });
