...
type HighlightedMessage = {
	id: string;
	/**
	 * Bumped on every highlight so that re-tapping the same reply re-triggers the flash even while the previous
	 * highlight is still active.
	 */
	key: number;
};
...
type MessageRepliesContextType = {
	...
	clearReply: () => void;
	/** Scroll the list to a message, if it's currently loaded, and flash it. No-op otherwise. */
...
export function useMessageReplies() {
	...
	if (!ctx) {
		throw new Error('useMessageReplies must be used within a MessageRepliesProvider');
	}
	return ctx;
}
...
export function MessageRepliesProvider({
	...
	const [replyTo, setReplyTo] = useState<ChatBskyConvoDefs.MessageView | null>(null);
	const [highlightedMessage, setHighlightedMessage] = useState<HighlightedMessage | null>(null);
