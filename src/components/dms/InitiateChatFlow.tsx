...
type Item = NewGroupChatItem | LabelItem | ProfileItem | EmptyItem | PlaceholderItem | ErrorItem;
...
export function InitiateChatFlow({
	...
	const [{ chatState, screenTitle, groupChatDids, groupChatProfiles, groupName }, dispatch] = useReducer(
		reducer,
		{
			chatState: ChatState.NEW_CHAT,
			screenTitle: title,
	...
	const items = useMemo(() => {
		let _items: Item[] = [];
		const checker = chatState === ChatState.NEW_GROUP_CHAT ? canBeAddedToGroup : canBeMessaged;
	...
	const showChatProfileTabs = chatState === ChatState.NEW_GROUP_CHAT && groupChatProfiles.length > 0;
						...
						<Dialog.FlatListFooter onLayout={(evt) => setFooterHeight(evt.nativeEvent.layout.height)}>
							<View style={[a.flex_row, a.align_center, a.justify_between]}>
								...
								>
									...
									{chatState !== ChatState.GROUP_NAME ? <ButtonIcon icon={ArrowRightIcon} size="md" /> : null}
								</Button>
							</View>
