...
export function SortableList<T>({
	...
	const getNewPosition = (state: { activeIndex: number; currentY: number; startY: number }) => {
		const translationY = state.currentY - state.startY;
		const rawNewPos = Math.round((state.activeIndex * itemHeight + translationY) / itemHeight);
	...
	return (
		<View
			style={[{ height: data.length * itemHeight, position: 'relative' }, t.atoms.bg_contrast_25]}
		...
		>
			{data.map((item, index) => {
				...
				const rawTranslationY = isActive ? dragState.currentY - dragState.startY : 0;
				...
				const dragHandle = (
					<div
						onPointerDown={(e: React.PointerEvent<HTMLDivElement>) => handlePointerDown(e, index)}
						style={{
							display: 'flex',
