...
/**
 * Tab component that automatically scrolls the selected tab into view - used for interests in the Find
 * Follows dialog, Explore screen, etc.
 */
export function InterestTabs({
	...
	function handleTabLayout(index: number, x: number, width: number) {
		if (!tabOffsets.length) {
			...
			if (pendingTabOffsets.current.filter((o) => !!o).length === interests.length) {
	...
	return (
		<View style={[a.relative, a.flex_row]}>
			<DraggableScrollView
				ref={listRef}
				contentContainerStyle={[a.gap_sm, { paddingHorizontal: gutterWidth }, contentContainerStyle]}
