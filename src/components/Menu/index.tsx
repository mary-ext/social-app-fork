...
export function Item({ children, label, onPress, style, destructive = false, ...rest }: ItemProps) {
	const t = useTheme();
	const { control } = useMenuContext();
	const { state: hovered, onIn: onMouseEnter, onOut: onMouseLeave } = useInteractionState();
	const { state: focused, onIn: onFocus, onOut: onBlur } = useInteractionState();
	...
	return (
		<DropdownMenu.Item asChild>
			<Pressable
				...
				{...webPointerProps}
			>
				<ItemContext.Provider value={{ disabled: Boolean(rest.disabled), destructive }}>
					{children}
...
export function LabelText({ children, style }: { children: React.ReactNode; style?: StyleProp<TextStyle> }) {
	const t = useTheme();
	return (
		<Text
			style={[
...
export function Divider() {
	...
	return (
		<DropdownMenu.Separator
			style={flatten([a.my_xs, t.atoms.bg_contrast_100, a.flex_shrink_0, { height: 1 }])}
		/>
	);
}
