...
/**
 * This uses `Portal` magic ✨ to render the icons and title correctly. ItemIcon and ItemText components get
 * teleported to the top row, leaving the rest of the children in the bottom row.
 */
...
export function Divider({ style }: ViewStyleProp) {
	const t = useTheme();
	return <View style={[a.border_t, t.atoms.border_contrast_low, a.w_full, a.my_sm, style]} />;
}
...
export function BadgeText({ children, style }: { children: React.ReactNode; style?: StyleProp<ViewStyle> }) {
	const t = useTheme();
	return (
		<Text
			style={[t.atoms.text_contrast_low, a.text_md, a.text_right, a.leading_snug, style]}
			numberOfLines={1}
		>
			{children}
		</Text>
