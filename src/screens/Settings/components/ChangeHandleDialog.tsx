				...
				{serviceInfoError ? (
					...
					<LayoutAnimationConfig skipEntering skipExiting>
						{page === 'provided-handle' ? (
							<Animated.View key={page} entering={undefined} exiting={undefined}>
								<ProvidedHandlePage serviceInfo={serviceInfo} goToOwnHandle={() => setPage('own-handle')} />
							</Animated.View>
						) : (
							<Animated.View key={page} entering={undefined} exiting={undefined}>
				...
				<Animated.View layout={undefined} style={[a.flex_1, a.gap_md]}>
					{verification.isVerified && verification.role === 'default' && (
						<Admonition type="error">
							<Trans>
								You are verified. You will lose your verification status if you change your handle.{' '}
								<InlineLinkText
									label={l({
										message: `Learn more`,
					...
					<Text>
						<Trans>
							...
							<Text style={[a.font_semi_bold]}>@{createFullHandle(subdomain, host)}</Text>
						</Trans>
					</Text>
					<Button
...
function OwnHandlePage({ goToServiceHandle }: { goToServiceHandle: () => void }) {
	...
	return (
		<View style={[a.flex_1, a.gap_lg]}>
			...
			<Animated.View layout={undefined} style={[a.flex_1, a.gap_md, a.overflow_hidden]}>
				<View>
					<TextField.LabelText>
				...
				{dnsPanel ? (
					<>
						...
						<View
							style={[t.atoms.bg_contrast_25, a.rounded_sm, a.p_md, a.border, t.atoms.border_contrast_low]}
						>
					...
					<>
						...
						<CopyButton
							...
							shape="rectangular"
							color="secondary"
							style={[a.px_md, a.border, t.atoms.border_contrast_low, t.atoms.bg_contrast_25]}
						>
			...
			<Animated.View layout={undefined}>
				{currentAccount?.handle?.endsWith('.bsky.social') && (
					<Admonition type="info" style={[a.mb_md]}>
						<Trans>
							Your current handle{' '}
							<Text style={[a.font_semi_bold]}>{sanitizeHandle(currentAccount?.handle || '', '@')}</Text> will
							automatically remain reserved for you. You can switch back to it at any time from this account.
						</Trans>
					</Admonition>
				)}
				<Button
					label={isVerified ? l`Update to ${domain}` : dnsPanel ? l`Verify DNS Record` : l`Verify Text File`}
					variant="solid"
					size="large"
					color="primary"
