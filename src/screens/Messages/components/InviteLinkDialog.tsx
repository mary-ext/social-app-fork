...
export function InviteLinkDialog({
	...
	switch (step) {
		case Step.INFO:
					...
					<View>
						<Text style={[a.text_md, t.atoms.text]}>
							<Trans>
								...
								can use the link and whether they need your approval. You can disable the link at any time.
							</Trans>
						</Text>
						<Text style={[a.mt_lg, a.text_md, t.atoms.text]}>
							<Trans>Your name, avatar, and the name of the group chat will be visible to everyone.</Trans>
						</Text>
					</View>
					<View style={[a.mt_4xl]}>
						<Button
			...
			break;
		case Step.GENERATE:
			header = joinLink && enabledStatus === 'enabled' ? l`Update invite link` : l`Generate invite link`;
			content = (
				<>
					<View>
						<Text style={[a.text_md, t.atoms.text]}>
					...
					<View style={[a.mt_4xl]}>
						<Button
							label={
								joinLink && enabledStatus === 'enabled' ? l`Update invite link` : l`Generate invite link`
							}
							color="primary"
							size="large"
							disabled={isSaving}
		...
		case Step.MANAGE: {
			...
			const currentOption = whoCanJoinOptions.find((o) => o.name === currentOptionName);
			...
			header = enabledStatus === 'enabled' ? l`Invite link` : l`Invite link disabled`;
			content = (
				<>
					<View style={[a.mt_lg]}>
						<CopyTextButton
						...
						>
							<Text
								...
								style={[
									a.mr_xs,
									a.text_md,
									enabledStatus === 'disabled' ? t.atoms.text_contrast_low : t.atoms.text,
								]}
							>
								{joinLinkURI}
							</Text>
		...
		case Step.CONFIRM_DISABLE:
			content = (
				<>
					...
					<Text style={[a.pb_2xl, a.text_center, a.text_sm, a.leading_snug, t.atoms.text]}>
						<Trans>
				...
				header={
					<View>
						<View style={[a.px_2xl, a.pt_xl]}>
							<Text style={[a.font_bold, a.text_2xl, a.mb_sm, t.atoms.text]}>{header}</Text>
						</View>
						<Dialog.Close />
					</View>
				}
