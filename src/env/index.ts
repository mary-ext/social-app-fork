...
/**
 * The semver version of the app, specified in our `package.json`.file. On iOs/Android, the native build
 * version is appended to the semver version, so that it can be used to identify a specific build.
 */
export const APP_VERSION = RELEASE_VERSION;

/** The short commit hash and environment of the current bundle. */
export const APP_METADATA = `${BUNDLE_IDENTIFIER.slice(0, 7)} (${import.meta.env.DEV ? 'dev' : 'prod'})`;

/** Web-specific platform detection */
export const IS_WEB_TOUCH_DEVICE = window.matchMedia('(pointer: coarse)').matches;
export const IS_WEB_MOBILE: boolean = window.matchMedia('only screen and (max-width: 1300px)')?.matches;
export const IS_WEB_MOBILE_IOS: boolean = /iPhone/.test(navigator.userAgent);
export const IS_WEB_MOBILE_ANDROID: boolean = /android/i.test(navigator.userAgent) && IS_WEB_TOUCH_DEVICE;
