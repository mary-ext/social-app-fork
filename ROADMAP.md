...
- [x] **`src/state/preferences/languages.tsx`** — keep `contentLanguages`, `postLanguage`, etc. Keep the `setAppLanguage` API wired to the English-only `AppLanguage` enum so the app-language selector remains easy to expand later
...
- [x] **App-language UI lives in more places than Settings.** Keep `src/components/AppLanguageDropdown.tsx` as a one-option `'English'` selector scaffold and keep its surviving consumers in Settings, logged-out right/nav surfaces, and the splash footer. The content-language picker (post-language detection, etc.) stays
